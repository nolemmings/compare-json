 ...
 *   output: stdout, // Output stream function, default `process.stdout.write`,
 *   error: stderr, // Output stream function, default `process.stderr.write`
 * }
 */
  ...
  compare.compareFiles(files, function(err, results) {
    ...
    _.forEach(results, function(result) {
      if (result.missingKeys && result.missingKeys.length > 0) {
        ...
        error(colors.info(colors.bold(result.file) + ' is complete\n'));
      }
    });
